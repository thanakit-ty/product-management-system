    ...
    <div class="page">
        ...
        <main class="card" aria-label="Login form">
            ...
            <form class="form" @submit.prevent="onSubmit">
                ...
                <div class="field">
                    ...
                    <div class="inputWrap">
                        ...
                        <input id="password" v-model="password" class="input" :type="showPassword ? 'text' : 'password'"
                            autocomplete="current-password" placeholder="••••••••" required :disabled="auth.loading" />
                ...
                <div v-if="auth.error" class="alert" role="alert">
                ...
                <div class="footerRow">
                    <a class="link" href="" @click.prevent="goRegister">Create account</a>
                    <a class="link muted" href="" @click.prevent="forgotPassword">Forgot password?</a>
                </div>
            </form>

            <footer class="fineprint">
                © {{ new Date().getFullYear() }} PMS • Secure sign-in
            </footer>
        </main>
    </div>
</template>
...
onMounted(() => {
    // optional: เติม email มาจาก query (เช่น หลัง register)
    if (route.query.email) email.value = String(route.query.email);
});

const canSubmit = computed(() => email.value.length > 3 && password.value.length >= 6);
...
function forgotPassword() {
    // TODO: ถ้าคุณจะทำ feature นี้ทีหลัง ให้เปลี่ยนไปหน้า /forgot-password
